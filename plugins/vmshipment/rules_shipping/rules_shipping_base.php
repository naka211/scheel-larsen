...
//            die;
			...
			if (($expr[0]=='\'' || $expr[0]=='"') && ($expr[0]==substr($expr,-1)) ) {
				...
				return $vals[strtolower($expr)];
                            ...
                            //Trieu add
                            preg_match_all("/\b[A-Z][\w]*/",$expr,$matches);
//                            $a = preg_replace("/\b\S{1,3}\b/", "", $expr);
//                            $a = str_replace(str_split('\+-*/()'), '', $a);
                            foreach($matches[0] as $word){
                                $wval = ($vals[lcfirst($word)])?$vals[lcfirst($word)]:0 ;
                                $expr = str_replace($word, $wval, $expr);
                            }
                            return $this->calculate_string($expr);
