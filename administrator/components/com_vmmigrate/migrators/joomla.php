...
class VMMigrateModelJoomla extends VMMigrateModelBase {
	...
	public static function getSteps() {
		...
		$steps[] = array('name'=>'joom_search'						,'default'=>1);
...
//			$record_group_mapping = array($record_group_map);
	...
	public function joom_sections() {
        ...
        foreach ($items as $i => $item) {
			...
			$record->title			= $item['title'];
			$record->alias			= $item['alias']; //$this->checkSlug('#__virtuemart_vendors_'.VMLANG,$slug,'slug',$srcid,'virtuemart_vendor_id');
			$record->description	= $item['description'];
			$record->published		= $item['published'];
			$record->access			= intval($item['access'])+1;
			//$record->params			= $item[''];
	...
	public function joom_images() {
		...
		$src = '/images';
		if (version_compare($this->joomla_version_dest, 3, '<')) {
			$bkup = '/images_'.JFactory::getDate()->toFormat('_%Y_%m_%d_%H_%M_%S');
		} else {
			$bkup = '/images_'.JFactory::getDate()->format('_%Y_%m_%d_%H_%M_%S');
		}
	...
	private function ImportMenuItems($menutype,$oldparentid=0,$newparentid=0,$path='') {
		$pk = 'id';
        //$excludeids = $this->getAlreadyTransferedIds();			// Load already transfered items
		$menuItemsFilter = "menutype='".$menutype."' AND parent='".$oldparentid."'";
        ...
        foreach ($items as $i => $item) {
			...
			$record->alias			= $item['alias'];
			$record->path			= ($path) ? $path.'/'.$item['alias'] : $item['alias'];
			...
			$record->component_id	= ($item['type']=='component') ? $this->getExtensionId($item['componentid']) : 0;
			$record->checked_out	= 0;
