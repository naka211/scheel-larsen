...
//		$config->set('browse_orderby_fields',$newOrderbyFields);
        ...
        foreach ($items as $i => $item) {
			...
			if (array_key_exists($item['payment_class'],$known_payment_methods)) {
				$query->clear()
					->select('extension_id')
					->from('#__extensions')
					->where('element='.$this->destination_db->q($known_payment_methods[$item['payment_class']]))
					->where('folder='.$this->destination_db->q('vmpayment'));
	...
	public function vm_shipping_rates() {
        ...
        foreach ($items as $i => $item) {
			...
			$record->virtuemart_vendor_id = $defaultVendor->virtuemart_vendor_id;
	...
	public function vm_category() {
        ...
        foreach ($items as $i => $item) {
			...
			if(!empty($item['category_full_image']) || !empty($item['category_thumb_image'])){
				$mediaid = $this->importMediaByType($item['category_name'],$item['category_full_image'],$item['category_thumb_image'],'category',$srcid);
				$record_media = new stdClass();
	...
	public function vm_userfield() {
        $pk = 'fieldid';
        $destination_table =& JTable::getInstance('userfields', 'Table');	//Load destination Table
        $excludeids = $this->getAlreadyTransferedIds();			// Load already transfered items
		$where = 'sys <> 1';
	...
	public function vm_shopper_vendor_xref() {
        ...
        foreach ($items as $i => $item) {
			...
			$record_user->virtuemart_vendor_id 			= ($user_is_vendor) ? $item['vendor_id'] : 0;
...
//			}
	...
	public function vm_product() {
        ...
        foreach ($items as $i => $item) {
			...
			$record_lang->product_name			 	= $item['product_name'];
			$record_lang->metadesc				 	= '';
			$record_lang->metakey				 	= '';
			$record_lang->customtitle			 	= '';
			$slug = JApplication::stringURLSafe($item['product_name']);
			$record_lang->slug = $this->checkSlug('#__virtuemart_products_'.$this->baseLanguageTable,$slug,'slug',$srcid,'virtuemart_product_id');
			...
			//Manufacturer
			$query = $this->source_db->getQuery(true);
			$query->select('manufacturer_id as virtuemart_manufacturer_id,product_id as virtuemart_product_id')
			...
			foreach ($related_products as $rel_product_id) {
				...
				if (version_compare($this->getVmVersion(),'2.9','<')) {
					...
					$custom_value->customfield_value		= $rel_product_id;
				}
			...
			try {
				...
				//$this->deleteRows('#__virtuemart_product_customfields',"virtuemart_product_id='".$item['product_id']."' AND virtuemart_custom_id<>2");
	...
	public function vm_product_type() {
        ...
        foreach ($items as $i => $item) {
			...
			$custom_parent->virtuemart_vendor_id 	= 1;
			...
			try {
				...
				$this->source_db->setQuery($query);
				$custom_products = $this->source_db->loadObjectList();
			} catch (Exception $e) {
			...
			try {
				...
				foreach ($custom_parameters as $child_parameter) {
					...
					$custom_child->virtuemart_vendor_id 	= 1;
					...
					switch ($child_parameter->parameter_type) {
						case 'I': $custom_child->field_type	= 'I'; break; //Interger
	...
	public function vm_orders() {
        ...
        foreach ($items as $i => $item) {
			...
			$order->virtuemart_vendor_id	= $item['vendor_id'];;
			...
			$ship_method_id = $ship_method_id[0];
			$order->virtuemart_shipmentmethod_id = $this->getShipmentMethodId($ship_method_id);
			...
			foreach ($items as $src_orderitem) {
				...
				$order_item->virtuemart_order_id 		= $src_orderitem->order_id;
	...
	private function getMediaIdByMeta($filename,$type){
		...
		$query->select('virtuemart_media_id')
			...
			->where('file_meta ='.$this->destination_db->q($filename))
	...
	private function importMediaByType($name,$filename,$alt_filename,$type,$ref,$description=''){
		...
		if (!in_array($ext,array('jpg','jpe','jpeg','gif','png','bmp','tif','tiff'))) {
			$filename = $alt_filename;
			$ext = strtolower(pathinfo($filename, PATHINFO_EXTENSION));
			if (!in_array($ext,array('jpg','jpe','jpeg','gif','png','bmp','tif','tiff','pdf'))) {
				return 0;
			}
		}
		...
		switch ($type) {
			case 'vendor':
				...
				$url = $dest_folder.basename($filename);
			...
			foreach ($attribute_values as $attribute_value) {
				...
				switch ($attribute_value->operator) {
					...
					case '=':
						...
						$finalprice = floatval($attribute_value->price);
						if (version_compare($this->getVmVersion(),'2.9','<')) {
							$custom_value->custom_price				= $finalprice-$baseprice;
						} else {
							$custom_value->customfield_price		= $finalprice-$baseprice;
						}
						break;
					case '+':
					default:
	...
	private function createCustomFieldIfNotExists($field_name,$isCart=true) {
		...
		$query->select('virtuemart_custom_id')
			...
			->where('field_type='.$db->q(($isCart)?'V':'S'))
...
//				->where('field_type='.$db->q('E'));
//			$db->setQuery($query);
			$jplugin = JPluginHelper::getPlugin('vmcustom', 'textinput');
			$jpluginid = $jplugin->id;
	...
	private function OrderItemAttributesToJson($order_item){
		...
		foreach ($attributesArray as $valueKeyVal) {
			...
			$query = $db->getQuery(true);
			$query->select('pc.virtuemart_customfield_id')
...
//		return json_encode($newAttributes,JSON_FORCE_OBJECT);
	...
	private function getShipmentMethodId($slug) {
		$query = $this->destination_db->getQuery(true);
