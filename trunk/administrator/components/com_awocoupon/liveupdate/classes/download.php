...
class LiveUpdateDownloadHelper
{
	...
	public static function download($url, $target) {
		...
		$post_data = awoLibrary::getliveupdateinfo();
		...
		if($fp !== false) {
			...
			while(!empty($adapters) && ($result === false)) {
				...
				{ # awo_code
					if(strtolower($method)!='getcurl') continue;
					$result = self::$method($url, $fp,false,$post_data);
					//$result = self::$method($url, $fp); # comment out
				}
				...
				if($result === true) {
					...
					{ # awo_code
						$test = file_get_contents($target,false,null,-1,200);
						if(substr($test,0,9)=='==error==') {
							JFactory::getApplication()->enqueueMessage( substr($test,9), 'error');
							return false;
						}
					}
		...
		if($result === false) {
			...
			{ # awo_code
				$string = self::getCURL($url, null,false,$post_data);
				if(substr($string,0,9)=='==error==') {
					JFactory::getApplication()->enqueueMessage( substr($string,9), 'error');
					return false;
				}
				$result = JFile::write($target, $string );
				//$result = JFile::write($target, self::downloadAndReturn($url) ); # comment out
			}
	 ...
	 */
	private static function &getCURL($url, $fp = null, $nofollow = false,$request = array())
	{
		$result = false;
		...
		{ # awo_code
			$request_string = '';
			if(!empty($request)) {
				foreach($request as $k=>$v) $request_string .= $k.'='.urlencode($v).'&';
