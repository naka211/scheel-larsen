...
class LiveUpdateINIHelper
{
	 ...
	 * @return	array	An associative array of sections, keys and values
	 */
	static function parse_ini_file_php($file, $process_sections = false, $rawdata = false) { # awo_code
		...
		$rawdata = !$rawdata ? file_get_contents($file) : $file;
		
		if( $rawdata && strstr($rawdata, '<?xml version="1.0" encoding="utf-8"?><update>') ) {
			$element = simplexml_load_string($rawdata);
			
			$rtn['software'] 		= @(string)$element->software;
			$rtn['version'] 		= @(string)$element->version;
			$rtn['link'] 			= @(string)$element->link;
			$rtn['date'] 			= @(string)$element->released;
			$rtn['releasenotes'] 	= '';
			$rtn['infourl'] 		= @(string)$element->infourl;
	...
	static function _parse_ini_file_php($file, $process_sections = false, $rawdata = false)
	{
